...
scoreboard players set @s draw_lock 1
tellraw @s ["[画笔] 已设置为yz平面锁定"]

# get id
execute as @e[tag=draw_pig] if score @s draw_id = @a[tag=draw_temp_player,limit=1] draw_id run tag @s add draw_temp_pig

# save lock pos
execute as @e[tag=draw_temp_pig] store result score @s draw_lock_num run data get entity @s Pos[0] 1000
